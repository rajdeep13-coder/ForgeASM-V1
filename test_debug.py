...
def run_step_trace(isa_name, code, max_steps=30):
    isa = ISA(isa_name)
    asm = Assembler(isa)
    ...
    for step in range(max_steps):
        ...
        opcode_len = {"risc1": 6, "risc2": 8, "risc3": 6, "cisc": 8}[isa_name]
        ...
        cpu.step()
        print(f"  Step {step+1:2d}: PC={pc:3d}→{cpu.get_pc():3d} [{inst_name:8s}] out='{cpu.output_buffer}' regs=[{regs_str}]")
    
    print(f"\nFinal: output='{cpu.output_buffer}' halted={cpu.halted}")
    print()


print("=" * 60)
print("DEBUG: RISC1 JMP $3")
print("=" * 60)
run_step_trace('risc1', "MOV $72\nJMP $3\nMOV $0\nOUT $1\nHALT")

print("=" * 60)
print("DEBUG: RISC1 alphabet_printout")
print("=" * 60)
with open('modules/demos/risc1/alphabet_printout.asm') as f:
    code = f.read()
run_step_trace('risc1', code, max_steps=50)

print("=" * 60)
print("DEBUG: RISC2 helloworld")  
print("=" * 60)
with open('modules/demos/risc2/helloworld.asm') as f:
    code = f.read()
run_step_trace('risc2', code, max_steps=30)
